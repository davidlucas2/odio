...
const BotName = 'Thominhaxbot'; 
const aktif = '08:00 - 22:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:nabuto\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=556195993572:+55 556195993572\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `VAI TARDE FDP @${num.split('@')[0]} 👺🖐🏻`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD THOMINHAX BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de THOMAS. Registre-se para fazer amizade com o bot thominhax por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar THOMINHAX|14\n\n──「 THOMINHAX BOT 」──`,
			...
			const ownerNumber = ["556195993572@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"556195993572@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["556195993572@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["556195993572@s.whatsapp.net"]//ubah nomor lo
			const premium = ["556195993572@s.whatsapp.net"]
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("bosta")){
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/THOMINHAXjokes.js');
		...
		case 'iri':
			...
			break
              ...
              case '(.aline farias)':
					...
					buffer = await getBuffer(`(https://www.universidadedoprazer.com/wp-content/uploads/2018/11/Aline-Faria-Nua-em-Ensaio-Sensual-Pelada-Download-Fotos-72.jpg)`)
    ...
    break
            ...
            case 'rapL':
				...
				case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar THOMINHAX|14`)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO THOMINHAX BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @SADFF-XL domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'textblue':
					...
					if (tels.ength > 10) return reply('O texto é longo, até 9 caracteres')
					reply(mess.wait)
					anu = await fetchJson(`http://melodicxt.herokuapp.com/api/txtcus tom?theme=blue_metal&text=${tels}&apiKey=administrator`, {method: 'get'})
					buff = await getBuffer(anu.result)
					client.sendMessage(from, buff, image, {quoted: mek})
					break
				case 'textsky':
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "THOMINHAX"🙄, chama ele aí ele diz que é "Shitpost"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT THOMINHAX ✨* 」\n*│+ wa.me/556195993572*╰──────*「 *THOMINHAX* 」*────*\n\n*_SE QUER SER ADMIN DO BOT THOMINHAX_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*@SADFF_XL'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* THOMINHAX\n*WPP:* wa.me/+556195993572\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
